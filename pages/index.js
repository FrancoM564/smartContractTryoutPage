...
export default function Home() {
  ...
  const [watermarkImageBytesArray, setWatermarkImageBytesArray] =
    useState(null);
  ...
  const publishProcess = async (key) => {
    ...
    var memoryBeforeWatermark = performance.memory.usedJSHeapSize;

    const { watermarkedAudio } = await helper.ocultarImagenEnAudio(
      file,
      watermarkImageFile
    );

    var memoryAfterWatermark = performance.memory.usedJSHeapSize;
    console.log(memoryBeforeWatermark, memoryAfterWatermark);

    const finishTimeWatermark = performance.now() - startTimeWaterMark;
    const memoryConsumedWatermark =
      memoryAfterWatermark - memoryBeforeWatermark;

    setTitle("Audio con marca", watermarkedAudio);

    setTitle("Aplicando encriptacion");

    const startTimeEncryption = performance.now();
    var memoryBeforeEncryption = performance.memory.usedJSHeapSize;

    const encryptedStr = await helper.applyAesEncryption(watermarkedAudio, key);

    var memoryAfterEncryption = performance.memory.usedJSHeapSize;

    const memoryConsumedEncryption = (
      (memoryAfterEncryption - memoryBeforeEncryption) /
      1048576
    ).toFixed(2);
    const finishTimeEncryption = performance.now() - startTimeEncryption;
    ...
    const startTimeSmartContractCreation = performance.now();

    const { buyAddress, reportAddress } = await deploySmartContract(
      fileHashAddress,
      imageHashAddress
    );

    const finishTimeSmartContractCreation =
      performance.now() - startTimeSmartContractCreation;
    ...
    console.log(
      "Tiempo marcaje de agua: ",
      finishTimeWatermark,
      "ms\nConsumo de memoria marca de agua: ",
      memoryConsumedWatermark,
      "B\nTiempo encriptacion: ",
      finishTimeEncryption,
      "ms\nConsumo de memoria encriptacion: ",
      memoryConsumedEncryption,
      "MB\nTiempo creación contratos: ",
      finishTimeSmartContractCreation,
      "ms"
    );

    helper.saveOnCSVLoadValues(
      file.name,
      finishTimeEncryption,
      finishTimeWatermark,
      finishTimeSmartContractCreation,
  ...
  const deployReportContract = async (buyContractAddress, owner) => {
    return new Promise(async (resolve, _) => {
      const metadataResponse = await fetch(
        "http://localhost:3000/metadataReport.json"
      );
  ...
  return (
    <div className="container text-center">
      <h1 className="fw-bold fs-1 ">{titletext}</h1>
      ...
      <div className="row container">
        ...
        <div className="col container p-5">
          ...
          <input
            ...
            type="file"
            onChange={({ target }) => {
              onFileSelectorChange2(target);
            }}
          />

          {watermarkImageBytesArray != null && (
            <h2 className="p-3">Imagen de marca de agua</h2>
          )}
          <div className="p-5">
            <img id="imagePreview" />
          </div>
